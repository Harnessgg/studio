...
import { buildWorkspaceFileUrl, classifyWorkspacePreviewKind } from "../studioWorkspace";
import {
...
function findLatestResolvedAssistantMessage(
  ...
  const latestAssistantMessageId = activeThread.latestTurn?.assistantMessageId ?? null;
  if (latestAssistantMessageId) {
    const matchingMessage = activeThread.messages.find(
      (message) => message.id === latestAssistantMessageId && message.role === "assistant",
    );
    if (matchingMessage && !matchingMessage.streaming) {
...
const LAST_EDITOR_KEY = LAST_EDITOR_STORAGE_KEY;
const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = LAST_INVOKED_SCRIPT_BY_PROJECT_STORAGE_KEY;
...
function isStudioSessionContextMessage(message: ChatMessage): boolean {
  return (
    ...
    message.text.trimStart().startsWith(STUDIO_SESSION_CONTEXT_PREFIX)
  );
}
...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const [_expandedWorkGroups, setExpandedWorkGroups] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    ...
    void queryClient.invalidateQueries({
      queryKey: projectQueryKeys.inspectWorkspace(activeProject.cwd),
    });
  }, [activeLatestTurn?.completedAt, activeProject?.cwd, latestTurnSettled, queryClient]);
  ...
  const activeWorkStartedAt =
    isSendBusy || isConnecting || isRevertingCheckpoint
      ? sendStartedAt
      : phase === "running"
        ? (activeLatestTurn?.startedAt ?? sendStartedAt)
        : null;
  ...
  const liveChatTimelineEntries = useMemo(
    () =>
  ...
  const liveChatTailKey = useMemo(() => {
    const lastEntry = liveChatTimelineEntries[liveChatTimelineEntries.length - 1];
    if (!lastEntry) {
      return "empty";
    }
    if (lastEntry.kind === "message") {
      return `${lastEntry.id}:${lastEntry.message.text.length}:${lastEntry.message.streaming ? "streaming" : "done"}`;
    }
    if (lastEntry.kind === "work") {
      return `${lastEntry.id}:${lastEntry.entry.label}:${lastEntry.entry.detail ?? ""}`;
    }
    return `${lastEntry.id}:${lastEntry.proposedPlan.updatedAt}`;
  }, [liveChatTimelineEntries]);
  ...
  const _completionDividerBeforeEntryId = useMemo(() => {
    if (!latestTurnSettled) return null;
    if (!activeLatestTurn?.startedAt) return null;
    if (!activeLatestTurn.completedAt) return null;
  ...
  const _onMessagesClickCapture = useCallback(
  ...
  const _onMessagesScroll = useCallback(() => {
    const scrollContainer = messagesScrollRef.current;
    if (!scrollContainer) return;
  ...
  const _onMessagesWheel = useCallback((event: React.WheelEvent<HTMLDivElement>) => {
    if (event.deltaY < 0) {
      pendingUserScrollUpIntentRef.current = true;
    }
  }, []);
  const _onMessagesPointerDown = useCallback((_event: React.PointerEvent<HTMLDivElement>) => {
    isPointerScrollActiveRef.current = true;
  }, []);
  const _onMessagesPointerUp = useCallback((_event: React.PointerEvent<HTMLDivElement>) => {
    isPointerScrollActiveRef.current = false;
  }, []);
  const _onMessagesPointerCancel = useCallback((_event: React.PointerEvent<HTMLDivElement>) => {
    isPointerScrollActiveRef.current = false;
  }, []);
  const _onMessagesTouchStart = useCallback((event: React.TouchEvent<HTMLDivElement>) => {
    const touch = event.touches[0];
    if (!touch) return;
    lastTouchClientYRef.current = touch.clientY;
  }, []);
  const _onMessagesTouchMove = useCallback((event: React.TouchEvent<HTMLDivElement>) => {
    const touch = event.touches[0];
    if (!touch) return;
  ...
  const _onMessagesTouchEnd = useCallback((_event: React.TouchEvent<HTMLDivElement>) => {
  ...
  const envMode: DraftThreadEnvMode = activeWorktreePath
    ...
    : isLocalDraftThread
      ...
      : "local";
  const inspectedWorkspace = workspaceQuery.data;
  const latestResolvedAssistantMessage = useMemo(
    () => findLatestResolvedAssistantMessage(activeThread),
    [activeThread],
  );
  const parsedStudioResult = useMemo(
    () =>
      latestResolvedAssistantMessage
        ? parseCodexStudioResult(latestResolvedAssistantMessage.text)
        : null,
    [latestResolvedAssistantMessage],
  );
  ...
  const previewFileUrl =
    preferredPreviewPath && activeProject?.cwd
  ...
  const openPreviewFileInEditor = useCallback(() => {
    if (!preferredPreviewPath || !activeProject?.cwd) {
      return;
    }
    const api = readNativeApi();
    if (!api) {
    ...
    void api.shell.openInEditor(
      `${activeProject.cwd}${activeProject.cwd.includes("\\") ? "\\" : "/"}${preferredPreviewPath.replaceAll("/", activeProject.cwd.includes("\\") ? "\\" : "/")}`,
      "file-manager",
    );
  ...
  useEffect(() => {
    ...
    const availablePreviewPaths = new Set([
      ...(inspectedWorkspace?.mediaEntries ?? []).map((entry) => entry.path),
      ...(inspectedWorkspace?.exportPaths ?? []),
    ]);
    if (!availablePreviewPaths.has(selectedPreviewPath)) {
      setSelectedPreviewPath(null);
    }
  }, [inspectedWorkspace?.exportPaths, inspectedWorkspace?.mediaEntries, selectedPreviewPath]);

  useEffect(() => {
    setSelectedPreviewPath(null);
  }, [activeProject?.cwd, threadId]);
  ...
  const sendPlainUserTurn = useCallback(
    async (options: {
      text: string;
      interactionMode: ProviderInteractionMode;
      errorMessage: string;
      ...
      try {
        ...
        setComposerDraftInteractionMode(threadIdForSend, options.interactionMode);
        ...
        setThreadError(threadIdForSend, err instanceof Error ? err.message : options.errorMessage);
        sendInFlightRef.current = false;
        resetSendPhase();
        return false;
      }
  ...
  const onSubmitPlanFollowUp = useCallback(
    async ({
      ...
      const sendOptions: Parameters<typeof sendPlainUserTurn>[0] = {
        text,
        interactionMode: nextInteractionMode,
      ...
      await sendPlainUserTurn(sendOptions);
    },
    [sendPlainUserTurn],
  );
  ...
  const _onToggleWorkGroup = useCallback((groupId: string) => {
    setExpandedWorkGroups((existing) => ({
      ...existing,
      [groupId]: !existing[groupId],
    }));
  }, []);
  const _onExpandTimelineImage = useCallback((preview: ExpandedImagePreview) => {
    setExpandedImage(preview);
  }, []);
  const expandedImageItem = expandedImage ? expandedImage.images[expandedImage.index] : null;
  const _onOpenTurnDiff = useCallback(
  ...
  const _onRevertUserMessage = (messageId: MessageId) => {
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-background text-muted-foreground/40">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="text-center">
            <p className="text-sm">Select an edit session or create a new one to get started.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="flex min-h-64 flex-1 flex-col overflow-hidden border-b border-border">
          <header
            className={cn(
        ...
        <div className="flex shrink-0 flex-col bg-background">
          <div className="flex items-center gap-3 border-b border-border px-3 py-2 sm:px-5">
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              {activeWorkElapsed ? (
                <span className="inline-flex w-[5.5rem] shrink-0 justify-center text-xs tabular-nums text-muted-foreground">
                  {activeWorkElapsed}
                </span>
              ) : null}
            </div>
          ...
          <div className={cn("px-3 pt-2 sm:px-5", isGitRepo ? "pb-1" : "pb-3 sm:pb-4")}>
            ...
            >
              <div
                className={`group rounded-2xl border bg-card transition-colors duration-200 focus-within:border-ring/45 ${
      ...
      >
        <DialogPopup className="flex h-[min(76vh,640px)] w-[min(92vw,760px)] max-w-none flex-col p-0">
          <DialogPanel className="flex min-h-0 flex-1 flex-col">
            <DialogHeader className="border-b border-border px-5 py-4">
              <DialogTitle>Live Codex Chat</DialogTitle>
              <DialogDescription>
                Live conversation and work log for this edit session.
              </DialogDescription>
            </DialogHeader>
            <div
              ref={liveChatScrollRef}
              className="min-h-0 flex-1 overflow-y-auto bg-muted/10 px-4 py-4 sm:px-5"
            >
              {liveChatTimelineEntries.length > 0 ? (
                <div className="mx-auto max-w-3xl space-y-4">
                  {liveChatTimelineEntries.map((entry) => {
                    if (entry.kind === "work") {
                      return (
                        <div
                          key={`live-chat-work:${entry.id}`}
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <span className="text-[10px] text-muted-foreground">
                              {formatTimestamp(entry.entry.createdAt)}
                            </span>
                          </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <span className="text-[10px] text-muted-foreground">
                              {formatTimestamp(entry.proposedPlan.createdAt)}
                            </span>
                          </div>
        ...
        >
          ...
          <div className="relative isolate z-10 max-h-[92vh] max-w-[92vw]">
            ...
            <img
              ...
              className="max-h-[86vh] max-w-[92vw] select-none rounded-lg border border-border/70 bg-background object-contain shadow-lg/5"
              draggable={false}
            />
        ...
        {activeQuestion.options.map((option, index) => {
          ...
          return (
            ...
            >
              ...
              {isSelected ? <CheckIcon className="size-3.5 shrink-0 text-info" /> : null}
            </button>
          );
        })}
      </div>
...
const _MessagesTimeline = memo(function MessagesTimeline({
  hasMessages,
  isWorking,
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-sm text-muted-foreground/30">
          Describe the video edit you want to start this session.
        </p>
      </div>
    );
  }
...
const _OpenInPicker = memo(function OpenInPicker({
  keybindings,
  availableEditors,
  openInCwd,
}: {
