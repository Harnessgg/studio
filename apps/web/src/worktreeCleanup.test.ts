import { ProjectId, ThreadId } from "@studio/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.harnessgg/studio/worktrees/studio-mvp/studio-4e609bb8",
    );
    expect(result).toBe("studio-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.harnessgg/studio\\worktrees\\studio-mvp\\studio-4e609bb8",
    );
    expect(result).toBe("studio-4e609bb8");
  });

  it("uses the final segment even when outside ~/.harnessgg/studio/worktrees", () => {
