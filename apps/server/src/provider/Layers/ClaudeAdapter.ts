...
function buildPromptText(input: ProviderSendTurnInput): string {
  const rawEffort = input.modelOptions?.claudeCode?.effort ?? null;
  const requestedEffort = trimOrNull(rawEffort);
  const claudeModel = input.model;
  const caps = getModelCapabilities("claudeCode", claudeModel);
  const promptEffort =
  ...
  return Effect.gen(function* () {
    ...
    const startSession: ClaudeAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const requestedEffort = trimOrNull(input.modelOptions?.claudeCode?.effort ?? null);
        const caps = getModelCapabilities("claudeCode", input.model);
        const effort =
          requestedEffort && hasEffortLevel(caps, requestedEffort) ? requestedEffort : null;
        const fastMode = input.modelOptions?.claudeCode?.fastMode === true && caps.supportsFastMode;
        const thinking =
