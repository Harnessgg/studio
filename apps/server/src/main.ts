...
const CliEnvConfig = Config.all({
  mode: Config.string("STUDIO_MODE").pipe(
    Config.option,
    Config.map(
  ...
  port: Config.port("STUDIO_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("STUDIO_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  stateDir: Config.string("STUDIO_STATE_DIR").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("STUDIO_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  authToken: Config.string("STUDIO_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("STUDIO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("STUDIO_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("Studio by HarnessGG running", {
      ...safeConfig,
...
const stateDirFlag = Flag.string("state-dir").pipe(
  Flag.withDescription("State directory path (equivalent to STUDIO_STATE_DIR)."),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to STUDIO_LOG_WS_EVENTS).",
  ),
  Flag.withAlias("log-ws-events"),
  Flag.optional,
);

export const studioCli = Command.make("studio", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  ...
  Command.withDescription("Run the Studio by HarnessGG server."),
