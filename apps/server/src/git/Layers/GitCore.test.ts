  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* createGitBranch({ cwd: tmp, branch: "@studio/server/feat/session" });
        yield* createGitBranch({ cwd: tmp, branch: "@studio/server/tmp-working" });
        yield* checkoutGitBranch({ cwd: tmp, branch: "@studio/server/tmp-working" });

        const renamed = yield* renameGitBranch({
          cwd: tmp,
          oldBranch: "@studio/server/tmp-working",
          newBranch: "@studio/server/feat/session",
        });

        expect(renamed.branch).toBe("@studio/server/feat/session-1");
        const branches = yield* listGitBranches({ cwd: tmp });
        expect(
          branches.branches.some((branch) => branch.name === "@studio/server/feat/session"),
        ).toBe(true);
        expect(
          branches.branches.some((branch) => branch.name === "@studio/server/feat/session-1"),
        ).toBe(true);
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("@studio/server/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* createGitBranch({ cwd: tmp, branch: "@studio/server/feat/session" });
        yield* createGitBranch({ cwd: tmp, branch: "@studio/server/feat/session-1" });
        yield* createGitBranch({ cwd: tmp, branch: "@studio/server/tmp-working" });
        yield* checkoutGitBranch({ cwd: tmp, branch: "@studio/server/tmp-working" });
        ...
        expect(renamed.branch).toBe("@studio/server/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:pingdotgg/studio.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:pingdotgg/studio.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "studio/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
