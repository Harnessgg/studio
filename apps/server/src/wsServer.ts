...
import {
  ensureHarnessWorkspaceScaffold,
  inspectWorkspace,
  readProjectInstructionExcerpt,
...
function formatWorkspaceListing(paths: ReadonlyArray<string>, limit: number): string {
  if (paths.length === 0) {
...
const WORKSPACE_FILE_ROUTE = "/workspace-file";
...
> {
  ...
  const toolStatuses = yield* providerHealth.getToolStatuses;
  ...
  const normalizeDispatchCommand = Effect.fnUntraced(function* (input: {
    ...
    const loadHarnessTurnPreamble = Effect.fnUntraced(function* (threadId: ThreadId) {
      ...
      const workspace = yield* Effect.tryPromise(() =>
        inspectWorkspace({ cwd: project.workspaceRoot }),
      ).pipe(Effect.catch(() => Effect.succeed(null)));
      const styleGuideContents = workspace?.styleGuideExcerpt ?? null;
      const agentsGuideContents = yield* Effect.tryPromise(() =>
        readProjectInstructionExcerpt(project.workspaceRoot, "AGENTS.md", 2_000),
      ).pipe(Effect.catch(() => Effect.succeed(null)));
      const mediaListing =
        workspace?.mediaEntries.map((entry: { path: string }) => entry.path) ?? [];

      return [
        "Studio by HarnessGG session context:",
        `Project root: ${project.workspaceRoot}`,
        "Use the harnessgg-kdenlive toolchain for all Kdenlive/video edit operations.",
        "Write exported renders into .harnessgg/exports and keep older exports intact.",
        "Put intermediate, scratch, and temporary files in .harnessgg/tmp/ instead of the project root.",
        "If STYLE.md exists, apply it to subtitles, transitions, color, audio, and export naming.",
        "End the final response with a machine-readable Studio trailer.",
        "Use exactly one of:",
        "  STUDIO_RESULT: DONE",
        ...
        "If DONE, the linked file must be the primary finished artifact relative to the project root.",
        styleGuideContents
          ? `STYLE.md excerpt:\n${styleGuideContents}`
          : "STYLE.md excerpt:\n  (not present)",
        agentsGuideContents
        ...
        formatWorkspaceListing(mediaListing, 20),
      ].join("\n\n");
    });
    ...
    if (input.command.type === "project.create") {
      ...
      yield* ensureHarnessProjectScaffold(workspaceRoot);
      return {
        ...input.command,
        workspaceRoot,
    ...
    const harnessPreamble = yield* loadHarnessTurnPreamble(turnStartCommand.threadId);
    const normalizedMessageText =
      harnessPreamble && turnStartCommand.message.text.trim().length > 0
        ? `${harnessPreamble}\n\nUser edit request:\n${turnStartCommand.message.text}`
        : turnStartCommand.message.text;

    const snapshot = yield* projectionReadModelQuery.getSnapshot();
    const thread = snapshot.threads.find((candidate) => candidate.id === turnStartCommand.threadId);
    ...
    if (project?.workspaceRoot) {
      ...
      const logFilePath = path.join(
        ...
        ".harnessgg",
        "logs",
        `${timestamp}_${turnStartCommand.threadId}.jsonl`,
      );
      const logEntry = {
        ...
        originalText: turnStartCommand.message.text,
      };
      yield* fileSystem
        .makeDirectory(path.dirname(logFilePath), { recursive: true })
        .pipe(Effect.catch(() => Effect.void));
      yield* fileSystem
        .writeFileString(logFilePath, `${JSON.stringify(logEntry)}\n`)
        .pipe(Effect.catch(() => Effect.void));
    }
  ...
  const httpServer = http.createServer((req, res) => {
    ...
    void Effect.runPromise(
      Effect.gen(function* () {
        ...
        if (url.pathname === WORKSPACE_FILE_ROUTE) {
          const cwd = url.searchParams.get("cwd");
          const relativePath = url.searchParams.get("path");
          if (!cwd || !relativePath) {
            respond(400, { "Content-Type": "text/plain" }, "Missing workspace file parameters");
            return;
          }
          ...
          const contentType = Mime.getType(target.absolutePath) ?? "application/octet-stream";
  ...
  const routeRequest = Effect.fnUntraced(function* (request: WebSocketRequest) {
    switch (request.body._tag) {
      ...
      case WS_METHODS.projectsInspectWorkspace: {
        const body = stripRequestTag(request.body);
        return yield* Effect.tryPromise({
          try: () => inspectWorkspace(body),
          catch: (cause) =>
            new RouteRequestError({
              message: `Failed to inspect workspace: ${String(cause)}`,
            }),
        });
      }
