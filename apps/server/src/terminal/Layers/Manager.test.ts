...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "studio-terminal-"));
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.STUDIO_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          STUDIO_PROJECT_ROOT: "/repo",
          STUDIO_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.STUDIO_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.STUDIO_WORKTREE_PATH).toBe("/repo/worktree-a");
