...
projectionLayer("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect("stores message attachment references without mutating payloads", () =>
    Effect.sync(() =>
      fs.mkdtempSync(path.join(os.tmpdir(), "studio-projection-attachments-")),
    ).pipe(
  ...
  it.effect("overwrites stored attachment references when a message updates attachments", () =>
    Effect.sync(() =>
      fs.mkdtempSync(path.join(os.tmpdir(), "studio-projection-attachments-overwrite-")),
    ).pipe(
      Effect.flatMap((stateDir) =>
        Effect.gen(function* () {
          const projectionPipeline = yield* OrchestrationProjectionPipeline;
  ...
  it.effect("does not persist attachment files when projector transaction rolls back", () =>
    Effect.sync(() =>
      fs.mkdtempSync(path.join(os.tmpdir(), "studio-projection-attachments-rollback-")),
    ).pipe(
      Effect.flatMap((stateDir) =>
        Effect.gen(function* () {
          const projectionPipeline = yield* OrchestrationProjectionPipeline;
  ...
  it.effect("removes unreferenced attachment files when a thread is reverted", () =>
    Effect.sync(() =>
      fs.mkdtempSync(path.join(os.tmpdir(), "studio-projection-attachments-revert-")),
    ).pipe(
      Effect.flatMap((stateDir) =>
        Effect.gen(function* () {
          const projectionPipeline = yield* OrchestrationProjectionPipeline;
          ...
          yield* appendAndProject({
            ...
            payload: {
              ...
              checkpointRef: CheckpointRef.makeUnsafe(
                "refs/studio/checkpoints/thread-revert-files/turn/1",
              ),
              status: "ready",
              files: [],
          ...
          yield* appendAndProject({
            ...
            payload: {
              ...
              checkpointRef: CheckpointRef.makeUnsafe(
                "refs/studio/checkpoints/thread-revert-files/turn/2",
              ),
              status: "ready",
              files: [],
  ...
  it.effect("removes thread attachment directory when thread is deleted", () =>
    Effect.sync(() =>
      fs.mkdtempSync(path.join(os.tmpdir(), "studio-projection-attachments-delete-")),
    ).pipe(
      Effect.flatMap((stateDir) =>
        Effect.gen(function* () {
          const projectionPipeline = yield* OrchestrationProjectionPipeline;
  ...
  it.effect("ignores unsafe thread ids for attachment cleanup paths", () =>
    Effect.sync(() =>
      fs.mkdtempSync(path.join(os.tmpdir(), "studio-projection-attachments-unsafe-")),
    ).pipe(
      Effect.flatMap((stateDir) =>
        Effect.gen(function* () {
          const projectionPipeline = yield* OrchestrationProjectionPipeline;
      ...
      Effect.gen(function* () {
        ...
        yield* appendAndProject({
          ...
          payload: {
            ...
            checkpointTurnCount: 1,
            checkpointRef: CheckpointRef.makeUnsafe(
              "refs/studio/checkpoints/thread-conflict/turn/1",
            ),
            status: "ready",
            files: [],
    ...
    Effect.gen(function* () {
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 1,
          checkpointRef: CheckpointRef.makeUnsafe("refs/studio/checkpoints/thread-revert/turn/1"),
          status: "ready",
          files: [],
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 2,
          checkpointRef: CheckpointRef.makeUnsafe("refs/studio/checkpoints/thread-revert/turn/2"),
          status: "ready",
          files: [],
...
it.effect("restores pending turn-start metadata across projection pipeline restart", () =>
  Effect.gen(function* () {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "studio-projection-pipeline-restart-"));
