...
export const DEFAULT_DEV_STATE_DIR = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".harnessgg/studio", "dev"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@studio/contracts",
    "--filter=@studio/web",
    "--filter=@studio/server",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=@studio/server"],
  "dev:web": ["run", "dev", "--filter=@studio/web"],
  "dev:desktop": ["run", "dev", "--filter=@studio/desktop", "--filter=@studio/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("STUDIO_PORT_OFFSET"),
  devInstance: optionalStringConfig("STUDIO_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid STUDIO_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `STUDIO_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric STUDIO_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed STUDIO_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      STUDIO_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://localhost:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      STUDIO_STATE_DIR: resolvedStateDir,
    };

    if (host !== undefined) {
      output.STUDIO_HOST = host;
    }

    if (authToken !== undefined) {
      output.STUDIO_AUTH_TOKEN = authToken;
    } else {
      delete output.STUDIO_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.STUDIO_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.STUDIO_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.STUDIO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.STUDIO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.STUDIO_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.STUDIO_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.STUDIO_MODE = "web";
      delete output.STUDIO_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read STUDIO_PORT_OFFSET/STUDIO_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("STUDIO_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv("STUDIO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
      logWebSocketEvents: readOptionalBooleanEnv("STUDIO_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.STUDIO_PORT)} webPort=${String(env.PORT)} stateDir=${String(env.STUDIO_STATE_DIR)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  stateDir: Flag.string("state-dir").pipe(
    Flag.withDescription("State directory path (forwards to STUDIO_STATE_DIR)."),
    Flag.withFallbackConfig(optionalStringConfig("STUDIO_STATE_DIR")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to STUDIO_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("STUDIO_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to STUDIO_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("STUDIO_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to STUDIO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("STUDIO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to STUDIO_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("STUDIO_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to STUDIO_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("STUDIO_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to STUDIO_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("STUDIO_PORT")),
  ),
